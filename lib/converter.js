...
const QUALITY_PRESETS = {
  ...
  'patch-clean': { minAreaPercent: 0, maxShapes: Infinity, precision: 8, nodeReduction: 0, patchAreaPercent: 0.002, protectThinPercent: 3.5 }
};
...
export function convertSvgToAlightXml(svgInput, rawOptions = {}) {
  ...
  const walk = (node, context, useStack = new Set()) => {
    ...
    const cumulativeOpacity = preserveFidelity
      ? context.opacity
    ...
    if (tag === 'use') {
      ...
      const useGroupPath = preserveFidelity && ownOpacity < 1
        ? [...(context.groupPath || []), opacityGroup(attrs.id || id || 'SVG Use')]
        : (context.groupPath || []);
      if (preserveFidelity && ownOpacity < 1) stats.groupOpacityPreserved++;
      walk(idNodes.get(id), {
    ...
    if (CONTAINER_TAGS.has(tag)) {
      ...
      if (node !== rootNode && ownOpacity < 1) {
        if (preserveFidelity) stats.groupOpacityPreserved++;
        else {
      ...
      const childGroupPath = preserveFidelity && node !== rootNode && ownOpacity < 1
    ...
    candidates.push({
      ...
      sceneClipPaths,
      layerOpacity: preserveFidelity ? ownOpacity : 1,
