...
function buildColorLayerXml(group, index, width, height, duration, fps, precision) {
  const raw = String(group.fillColor || '#ff000000').replace('#', '').toUpperCase();
  const visible = raw.length === 8 && raw.startsWith('FF') ? `#${raw.slice(2)}` : `#${raw}`;
  // Follow the structure emitted by real Alight Motion group scenes:
  // neutral embedScene fill, empty nested scene title, ASCII-safe label.
  const label = `Color ${String(index + 1).padStart(2, '0')} - ${visible}`;
...
function buildColorGroupsLayerXml(units, width, height, duration, fps, precision, rootOpacity = 1) {
  ...
  return [
    `  <embedScene id="300000001" label="SVG Color Groups" startTime="0" endTime="${duration}" fillType="intrinsic" outTime="${duration}" mediaFillMode="fill">`,
    '    <transform>',
    `      <location value="${fmt(width / 2)},${fmt(height / 2)},0.000000" />`,
    ...(rootOpacity < 1 ? [`      <opacity value="${fmt(rootOpacity, precision)}" />`] : []),
    '    </transform>',
...
function validateAlightImportXml(xml) {
  ...
  if (!/<scene\b[^>]*\btotalTime="\d+"[^>]*\bfps="\d+"/.test(source)) {
    fail('Root scene Alight Motion tidak lengkap.');
  }
  if (/[\u0000-\u0008\u000B\u000C\u000E-\u001F]/.test(source)) {
    fail('XML mengandung control character ilegal.');
  }
  if (/\b(?:NaN|Infinity|undefined)\b/.test(source)) {
    fail('XML mengandung nilai invalid.');
  }

  const ids = [];
  for (const match of source.matchAll(/<(?:shape|embedScene)\b[^>]*\bid="(\d+)"/g)) {
    ids.push(match[1]);
  }
  if (new Set(ids).size !== ids.length) {
    fail('XML mengandung ID layer duplikat.');
  }

  for (const match of source.matchAll(/<embedScene\b[^>]*>/g)) {
    if (!/\boutTime="\d+"/.test(match[0])) {
      fail('embedScene tidak memiliki outTime.');
    }
  }
...
export function convertSvgToAlightXml(svgInput, rawOptions = {}) {
  ...
  const walk = (node, context, useStack = new Set()) => {
    ...
    candidates.push({
      ...
      sceneClipPaths,
      colorGroupingUnsafe: Boolean(
        activeMaskIds.length ||
        (style.filter && style.filter !== 'none') ||
        (gradId && !gradient) ||
        ['marker-start', 'marker-mid', 'marker-end'].some((key) => style[key] && style[key] !== 'none') ||
        (style['mix-blend-mode'] && !['normal', 'unset', 'initial'].includes(String(style['mix-blend-mode']).toLowerCase()))
      ),
  ...
  if (options.quality === 'color-group') {
    ...
    const eligibleColorKey = (shape) => {
      if (shape.colorGroupingUnsafe) return null;
    ...
    const scene = [
      `<?xml version='1.0' encoding='UTF-8' ?>`,
      `<!-- Color Groups v2.2.1: Maximum Fidelity geometry parser + Alight-import-compatible color group wrappers. -->`,
      `<scene title="${xmlEscape(options.title)}" width="${width}" height="${height}" exportWidth="${width}" exportHeight="${height}" precompose="dynamicResolution" bgcolor="#00000000" totalTime="${options.duration}" fps="${options.fps}" modifiedTime="${Date.now()}" amver="1028425" ffver="106" am="com.alightcreative.motion/5.0.273.1028425" amplatform="android" retime="freeze" retimeAdaptFPS="false">`,
      layerXml,
      `</scene>`
    ].join('\n');

    validateAlightImportXml(scene);
    stats.outputBytes = Buffer.byteLength(scene, 'utf8');
