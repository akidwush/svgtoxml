...
function buildFidelityReport(stats, sourceFeatures, quality) {
  ...
  if (quality === 'color-group') {
    add(
      'color-group-merge',
      stats.mergedShapes,
      'Color Groups sengaja menggabungkan shape dengan fill solid yang sama menjadi satu layer warna.'
    );
    add(
      'color-group-zorder',
      stats.zOrderBarriers,
      'Warna yang muncul terpisah oleh warna lain digabung ke satu layer; urutan overlap lintas warna dapat berbeda.'
    );
  }
...
export function convertSvgToAlightXml(svgInput, rawOptions = {}) {
  ...
  if (options.quality === 'color-group') {
    ...
    for (const shape of kept) {
      ...
      const key = eligibleColorKey(shape);
      if (!key) {
        fallback.push({
          ...shape,
          primitiveKind: null,
      ...
      group.localPaths.push(localPath);
      group.sourceOrders.push(shape.sourceOrder);
      group.sourceLabels.push(shape.sourceLabel);
    ...
    for (const group of groups.values()) {
      const orders = [...group.sourceOrders].sort((a, b) => a - b);
      let segments = orders.length ? 1 : 0;
      for (let i = 1; i < orders.length; i++) {
        if (orders[i] !== orders[i - 1] + 1) segments++;
      }
      const barriers = Math.max(0, segments - 1);
      stats.zOrderBarriers += barriers;
      stats.mergedShapes += Math.max(0, group.localPaths.length - 1);
      if (!barriers) stats.safeColorMerges += Math.max(0, group.localPaths.length - 1);

      const labelColor = group.fillColor.replace(/^#ff/i, '#');
      groupedShapes.push({
        sourceOrder: group.firstOrder,
        sourceLabel: `Color ${String(++groupIndex).padStart(2, '0')} · ${labelColor}`,
        sourceTag: 'color-group',
        primitiveKind: null,
        area: 0,
        bounds: unionBounds(group.bounds),
        scenePath: '',
        localPath: group.localPaths.join(''),
        ...
        layerOpacity: 1,
        groupPath: [],
        sortOrder: group.lastOrder,
        groupedSourceCount: group.localPaths.length
      });
    }

    const outputLayers = [...groupedShapes, ...fallback]
      .sort((a, b) => (a.sortOrder ?? a.sourceOrder) - (b.sortOrder ?? b.sourceOrder));

    stats.colorGroups = groupedShapes.length;
    stats.outputShapes = outputLayers.length;
    stats.missingShapes = Math.max(0, kept.length - serializedSources);

    if (fallback.length) {
      warnings.add(
        `Color Groups mempertahankan ${fallback.length} layer kompleks secara terpisah karena gradient/stroke/transparency/clip tidak aman digabung hanya berdasarkan warna.`
      );
    }
    if (stats.zOrderBarriers > 0) {
      warnings.add(
        `${stats.zOrderBarriers} pemisah z-order terdeteksi saat warna yang sama muncul di beberapa posisi. Engine tetap memaksa 1 warna solid = 1 layer sesuai mode Color Groups.`
      );
    }

    const fidelity = buildFidelityReport(stats, sourceFeatures, options.quality);
    const layerXml = buildLosslessLayerXml(
      outputLayers, width, height, options.duration, options.fps, options.precision, rootOpacity
    );
    const scene = [
      `<?xml version='1.0' encoding='UTF-8' ?>`,
      `<!-- Color Groups: Maximum Fidelity geometry parser + exact solid-fill grouping. -->`,
      `<scene title="${xmlEscape(options.title)}" width="${width}" height="${height}" exportWidth="${width}" exportHeight="${height}" precompose="dynamicResolution" bgcolor="#00000000" totalTime="${options.duration}" fps="${options.fps}" modifiedTime="${Date.now()}" amver="1028425" ffver="106" am="com.alightcreative.motion/5.0.273.1028425" amplatform="android" retime="freeze" retimeAdaptFPS="false">`,
      layerXml,
      `</scene>`
    ].join('\n');
    ...
    return {
      ...
      grouping: {
        ...
        groupedColors: groupedShapes.length,
        groupedLayers: groupedShapes.length,
        fallbackLayers: fallback.length,
        outputLayers: outputLayers.length,
      ...
      profile: {
        ...
        engine: 'color-groups',
        groupedByColor: true,
        colorGroups: groupedShapes.length,
