...
export function convertSvgToAlightXml(svgInput, rawOptions = {}) {
  ...
  const walk = (node, context, useStack = new Set()) => {
    ...
    if (gradId) {
      ...
      if (gradient) {
        stats.gradients++;
        // Mode v1.4 wajib 1 group = 1 warna solid. Gradient diratakan menjadi
        // warna representatif midpoint agar sebuah group tidak memiliki paint campuran.
        ...
        if (gradient.reducedStops) stats.gradientStopsReduced++;
      } else {
    ...
    // Stroke sengaja tidak ditulis ke XML. Jika elemen hanya punya stroke, elemen
    // tersebut dilewati agar output benar-benar bebas path-stroke.
    if (!fillColor) {
    ...
    const beforeNodes = pathNodeCount(transformed);
