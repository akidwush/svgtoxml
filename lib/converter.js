...
function buildLosslessLayerXml(shapes, width, height, duration, fps, precision) {
  const inner = shapes.map((shape, index) => shape.localClipPaths?.length
...
export function convertSvgToAlightXml(svgInput, rawOptions = {}) {
  ...
  for (const child of rootNode.children || []) {
    walk(child, {
      style: rootStyle, opacity: rootOpacity, transforms: [], viewportOverride: null,
      viewport: userViewport, clipIds: [], maskIds: []
    });
  }
  ...
  if (options.quality === 'optimized') {
    ...
    return {
      ...
      profile: {
        version: '1.8.0', quality: 'optimized', groupedByColor: 'z-order-safe',
