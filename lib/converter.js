...
function pruneMicroSubpaths(d, minArea = 0, protectLongDimension = Infinity, precision = 4) {
  ...
  if (parts.length <= 1) {
    ...
    if (w * h < minArea && Math.max(w, h) < protectLongDimension) {
  ...
  for (const part of parts) {
    ...
    const h = Math.max(0, b[3] - b[1]);
    const area = w * h;
    const isMicro = area < minArea && Math.max(w, h) < protectLongDimension;
    if (isMicro) {
      removed++;
...
const QUALITY_PRESETS = {
  ...
  // New phone-first profile: remove micro-islands, simplify contours moderately,
  // then merge same-style paths only when crossing z-order is provably harmless
  // by conservative bounding-box overlap checks.
  optimized: {
    minAreaPercent: 0.0004, maxShapes: 3000, precision: 4, nodeReduction: 38,
    microDetailPercent: 0.0015, maxOutputGroups: 320
  }
};
...
function resolveQualityOptions(rawOptions = {}) {
  ...
  if (quality === 'optimized') {
    return {
      ...
      maxShapes: clamp(Math.round(finiteNumber(pick('maxShapes'), preset.maxShapes)), 1, 10000),
      precision: clamp(Math.round(finiteNumber(pick('precision'), preset.precision)), 2, 8),
      geometryPrecision: clamp(Math.round(finiteNumber(pick('precision'), preset.precision)) + 3, 4, 10),
      nodeReduction: clamp(finiteNumber(pick('nodeReduction'), preset.nodeReduction), 0, 70),
      microDetailPercent: clamp(finiteNumber(rawOptions.microDetailPercent, preset.microDetailPercent), 0, 0.25),
      maxOutputGroups: clamp(Math.round(finiteNumber(rawOptions.maxOutputGroups, preset.maxOutputGroups)), 20, 1200),
      groupByColor: true,
      removeStrokes: false,
...
function buildFidelityReport(stats, sourceFeatures, quality) {
  ...
  if (quality !== 'lossless') {
    add('non-fidelity-mode', 1, 'Mode ini mengoptimalkan atau menggabungkan geometri.');
  }
  return {
...
export function convertSvgToAlightXml(svgInput, rawOptions = {}) {
  ...
  const walk = (node, context, useStack = new Set()) => {
    ...
    const cumulativeOpacity = options.quality === 'lossless'
      ? context.opacity
      : context.opacity * ownOpacity;
    if (cumulativeOpacity <= 0 || ownOpacity <= 0) return;
    const opacityGroup = (label) => ({
      token: opacityGroupToken++,
      label: `${String(label || 'SVG Group').slice(0, 72)} · Opacity`,
      opacity: ownOpacity
    });
    ...
    if (tag === 'use') {
      ...
      const useGroupPath = options.quality === 'lossless' && ownOpacity < 1
        ? [...(context.groupPath || []), opacityGroup(attrs.id || id || 'SVG Use')]
        : (context.groupPath || []);
      if (options.quality === 'lossless' && ownOpacity < 1) stats.groupOpacityPreserved++;
      walk(idNodes.get(id), {
    ...
    if (CONTAINER_TAGS.has(tag)) {
      ...
      if (node !== rootNode && ownOpacity < 1) {
        if (options.quality === 'lossless') stats.groupOpacityPreserved++;
        else {
          stats.groupOpacityFlattened++;
          warnings.add('Opacity pada group SVG diratakan ke child layer; area child yang saling overlap dapat berbeda dari hasil compositing group SVG.');
        }
      }
      ...
      const childGroupPath = options.quality === 'lossless' && node !== rootNode && ownOpacity < 1
        ? [...(context.groupPath || []), opacityGroup(attrs.id || attrs['data-name'] || tag)]
        : (context.groupPath || []);
      const childContext = {
    ...
    if (gradId) {
      ...
      if (gradient) {
        ...
        if (options.quality === 'lossless' || options.quality === 'optimized') {
    ...
    if (!fillColor && !((options.quality === 'lossless' || options.quality === 'optimized') && strokeColor && strokeWidth > 0)) {
    ...
    if ((options.quality === 'lossless' || options.quality === 'optimized') && fillRule === 'evenodd') {
    ...
    if ((options.quality === 'lossless' || options.quality === 'optimized') && strokeColor && strokeWidth > 0) {
    ...
    candidates.push({
      ...
      scenePath,
      fillColor,
      gradient: (options.quality === 'lossless' || options.quality === 'optimized') ? gradient : null,
      fillRule,
      strokeColor: (options.quality === 'lossless' || options.quality === 'optimized') ? strokeColor : null,
      strokeWidth: (options.quality === 'lossless' || options.quality === 'optimized') ? strokeWidth : 0,
      strokeJoin: String(style['stroke-linejoin'] || 'miter').toLowerCase(),
      strokeLineCap: lineCap,
      strokeDashArray: dashArray,
      sceneClipPaths,
      layerOpacity: options.quality === 'lossless' ? ownOpacity : 1,
  ...
  for (const child of rootNode.children || []) {
    walk(child, {
      style: rootStyle, opacity: options.quality === 'lossless' ? 1 : rootOpacity,
  ...
  if (options.quality === 'optimized') {
    const microArea = sceneArea * (options.microDetailPercent / 100);
    const protectLongDimension = Math.max(width, height) * 0.035;
    const prepared = [];

    for (const shape of kept) {
      let pruned;
      try {
        pruned = pruneMicroSubpaths(shape.scenePath, microArea, protectLongDimension, options.precision);
      } catch {
        pruned = { path: shape.scenePath, removed: 0, removedNodes: 0 };
      }
      stats.microSubpathsRemoved += pruned.removed || 0;
      stats.microNodesRemoved += pruned.removedNodes || 0;
      if (!pruned.path) {
        stats.removedMicroShapes++;
        continue;
      }

      let reducedPath = pruned.path;
      const before = pathNodeCount(pruned.path);
      try {
        reducedPath = reducePathNodes(pruned.path, options.nodeReduction, options.precision);
        const after = pathNodeCount(reducedPath);
        if (after < before) stats.nodeReductionReducedShapes++;
        else stats.nodeReductionUnchangedShapes++;
      } catch {
        reducedPath = serializePathForAlight(pruned.path, options.precision);
        stats.nodeReductionFallbackShapes++;
      }
      const bounds = safeBounds(reducedPath) || shape.bounds;
      if (!bounds) continue;
      prepared.push({ ...shape, reducedPath, bounds, styleKey: optimizedStyleKey(shape) });
    }

    // Safe merge: a later same-style shape may move backward into an existing
    // group only when it does not overlap any original shape it would cross.
    // This preserves visible stacking while still collapsing thousands of tiny
    // auto-vector islands that live in separate, non-overlapping regions.
    const safeGroups = [];
    const styleGroupIndexes = new Map();
    const orderIndex = new Map(prepared.map((shape, index) => [shape.sourceOrder, index]));
    for (let currentIndex = 0; currentIndex < prepared.length; currentIndex++) {
      const shape = prepared[currentIndex];
      let target = null;
      let targetIndex = -1;
      const sameStyle = styleGroupIndexes.get(shape.styleKey) || [];
      // Search only the most recent same-style runs. This keeps the conservative
      // overlap test fast even for 1k-5k path auto-vector SVGs on Vercel/phones.
      let tried = 0;
      for (let si = sameStyle.length - 1; si >= 0 && tried < 12; si--, tried++) {
        const gi = sameStyle[si];
        const group = safeGroups[gi];
        if (!group || shape.gradient) continue;
        const startIndex = (orderIndex.get(group.firstOrder) ?? -1) + 1;
        let blocked = false;
        for (let j = Math.max(0, startIndex); j < currentIndex; j++) {
          const other = prepared[j];
          if (group.orders.has(other.sourceOrder)) continue;
          if (boundsOverlap(shape.bounds, other.bounds, 0.35)) {
            blocked = true;
            break;
          }
        }
      ...
      if (!target) {
        target = {
          styleKey: shape.styleKey, firstOrder: shape.sourceOrder, orders: new Set(),
          shapes: [], areaSum: 0, fillColor: shape.fillColor, gradient: shape.gradient,
          strokeColor: shape.strokeColor, strokeWidth: shape.strokeWidth, strokeJoin: shape.strokeJoin
        };
        targetIndex = safeGroups.length;
        safeGroups.push(target);
        if (!styleGroupIndexes.has(shape.styleKey)) styleGroupIndexes.set(shape.styleKey, []);
        styleGroupIndexes.get(shape.styleKey).push(targetIndex);
      } else {
        stats.safeColorMerges++;
      }
      target.orders.add(shape.sourceOrder);
      target.shapes.push(shape);
      const b = shape.bounds;
      target.areaSum += Math.max(0, (b[2] - b[0]) * (b[3] - b[1]));
    }

    stats.optimizedGroupsBeforeCap = safeGroups.length;
    let groupsToEmit = safeGroups;
    if (safeGroups.length > options.maxOutputGroups) {
      const keep = new Set([...safeGroups]
        .sort((a, b) => b.areaSum - a.areaSum || a.firstOrder - b.firstOrder)
        .slice(0, options.maxOutputGroups));
      groupsToEmit = safeGroups.filter((g) => keep.has(g));
      stats.optimizedGroupsDropped = safeGroups.length - groupsToEmit.length;
      warnings.add(`${stats.optimizedGroupsDropped} group mikro terkecil dibuang agar output tidak melebihi ${options.maxOutputGroups} layer pada mode AM Optimized.`);
    }

    const groups = [];
    for (const group of groupsToEmit) {
      const ub = unionBounds(group.shapes.map((shape) => shape.bounds));
      if (!ub) continue;
      const cx = (ub[0] + ub[2]) / 2;
      const cy = (ub[1] + ub[3]) / 2;
      const localParts = [];
      for (const shape of group.shapes) {
        try {
          const translated = svgpath(shape.reducedPath).translate(-cx, -cy).abs().toString();
          localParts.push(serializePathForAlight(translated, options.precision));
        } catch {
          stats.nodeReductionFallbackShapes++;
        }
      }
      ...
      groups.push({
        cx, cy, localPath, firstOrder: group.firstOrder, sourceCount: group.shapes.length,
        fillColor: group.fillColor, gradient: group.gradient, strokeColor: group.strokeColor,
        strokeWidth: group.strokeWidth, strokeJoin: group.strokeJoin, areaSum: group.areaSum
      });
    }
    groups.sort((a, b) => a.firstOrder - b.firstOrder);
    stats.colorGroups = groups.length;
    stats.outputShapes = groups.length;
    stats.mergedShapes = Math.max(0, prepared.length - groups.length);
    if (stats.microSubpathsRemoved > 0) {
      warnings.add(`${stats.microSubpathsRemoved} subpath/titik mikro dibuang (${stats.microNodesRemoved} node) untuk meringankan XML.`);
    }
    if (stats.nodeReductionFallbackShapes > 0) {
      warnings.add(`${stats.nodeReductionFallbackShapes} contour tidak aman direduksi; geometri aslinya dipertahankan.`);
    }

    const shapeXml = groups.map((group, i) => buildOptimizedGroupXml(
      group, i, width, height, options.duration, options.fps, options.precision
    )).join('\n');
    const scene = [
      `<?xml version='1.0' encoding='UTF-8' ?>`,
      `<!-- AM Optimized: micro-path cleanup + conservative z-order-safe grouping. -->`,
      `<scene title="${xmlEscape(options.title)}" width="${width}" height="${height}" exportWidth="${width}" exportHeight="${height}" precompose="dynamicResolution" bgcolor="#00000000" totalTime="${options.duration}" fps="${options.fps}" modifiedTime="${Date.now()}" amver="1028425" ffver="106" am="com.alightcreative.motion/5.0.273.1028425" amplatform="android" retime="freeze" retimeAdaptFPS="false">`,
      shapeXml,
      `</scene>`
    ].filter(Boolean).join('\n');
    stats.outputBytes = Buffer.byteLength(scene, 'utf8');
    return {
      xml: scene, width, height, options, stats, warnings: [...warnings],
      profile: {
        version: '1.9.0', quality: 'optimized', groupedByColor: 'z-order-safe',
        preservesVisibleZOrder: true, nativeStroke: true, microDetailPercent: options.microDetailPercent,
        maxOutputGroups: options.maxOutputGroups, nodeReduction: options.nodeReduction
      },
      sourceFeatures,
      fidelity: buildFidelityReport(stats, sourceFeatures, options.quality)
    };
  }

  // Legacy modes intentionally keep the old global same-color grouping behavior.
  ...
  for (const group of colorMap.values()) {
    ...
    // Reduce each source contour independently before joining same-color shapes.
    // The previous implementation reduced the entire merged multi-subpath group
    // in one call: one malformed/unsupported contour made the whole color group
    // fall back to the unreduced path and produced one noisy warning per color.
    const localParts = [];
    for (const shape of group.shapes) {
      const translated = svgpath(shape.scenePath)
        .translate(-cx, -cy)
        .abs()
        .round(options.precision)
        .toString();
      const before = pathNodeCount(translated);
      let part;
      try {
        part = reducePathNodes(translated, options.nodeReduction, options.precision);
        const after = pathNodeCount(part);
        if (after < before) stats.nodeReductionReducedShapes++;
        else stats.nodeReductionUnchangedShapes++;
      } catch {
        part = serializePathForAlight(translated, options.precision);
        stats.nodeReductionFallbackShapes++;
      }
  ...
  if (stats.nodeReductionFallbackShapes > 0) {
    warnings.add(`${stats.nodeReductionFallbackShapes} shape tidak dapat direduksi dengan aman; path asli dipertahankan hanya untuk shape tersebut.`);
  }
