...
function optimizedStyleKey(shape) {
  ...
  return [
    ...
    String(shape.strokeLineCap || 'butt').toLowerCase(),
    String(shape.strokeDashArray || 'none').toLowerCase(),
    String(shape.fillRule || 'nonzero').toLowerCase()
  ].join('|');
}

function colorGroupingKey(shape) {
  if (
    shape.gradient ||
    (shape.localClipPaths && shape.localClipPaths.length) ||
    !isOpaqueArgb(shape.fillColor) ||
    !isOpaqueArgb(shape.strokeColor)
  ) {
    return `isolated:${shape.sourceOrder}`;
  }
  return optimizedStyleKey(shape);
}

function groupLosslessShapesByColor(shapes) {
  const groups = new Map();
  const order = [];
  const closedKeys = new Set();
  let lastKey = null;
  let zOrderBarriers = 0;

  for (const shape of shapes) {
    const key = colorGroupingKey(shape);
    if (lastKey != null && key !== lastKey) closedKeys.add(lastKey);
    if (key !== lastKey && closedKeys.has(key)) zOrderBarriers++;
    lastKey = key;
    ...
    if (!group) {
      group = {
        ...
        localClipPaths: shape.localClipPaths || [],
    ...
    group.parts.push(shape.localPath);
    if (group.bounds && shape.bounds) group.bounds = unionBounds([group.bounds, shape.bounds]);
  }

  const output = order.map((key, index) => {
    const group = groups.get(key);
    const color = group.fillColor || group.strokeColor || '#ff000000';
    return {
      sourceOrder: group.firstOrder,
      sourceLabel: colorLabel(color, index),
      sourceTag: 'color-group',
      primitiveKind: null,
      area: group.bounds ? Math.max(0, group.bounds[2] - group.bounds[0]) * Math.max(0, group.bounds[3] - group.bounds[1]) : 0,
      bounds: group.bounds,
      scenePath: '',
      localPath: group.parts.join(''),
      fillColor: group.fillColor,
      gradient: group.gradient,
      fillRule: group.fillRule,
      strokeColor: group.strokeColor,
      strokeWidth: group.strokeWidth,
      strokeJoin: group.strokeJoin,
      strokeLineCap: group.strokeLineCap,
      strokeDashArray: group.strokeDashArray,
      localClipPaths: group.localClipPaths,
      layerOpacity: group.layerOpacity,
      groupPath: [],
  ...
  return { groups: output, zOrderBarriers };
}
...
function buildLosslessLayerXml(shapes, width, height, duration, fps, precision, rootOpacity = 1, layerLabel = 'SVG Lossless Layer') {
  const tree = buildLosslessTree(shapes);
  const inner = buildLosslessTreeXml(tree.children, 0, width, height, duration, fps, precision);
  return [
    `  <embedScene id="300000001" label="${xmlEscape(layerLabel)}" startTime="0" endTime="${duration}" fillType="intrinsic" mediaFillMode="fill">`,
    '    <transform>',
    `      <location value="${fmt(width / 2)},${fmt(height / 2)},0.000000" />`,
...
function resolveQualityOptions(rawOptions = {}) {
  ...
  const aliases = {
    ...
    'group-by-color': 'color-groups',
    'color-layers': 'color-groups'
  };
...
function buildFidelityReport(stats, sourceFeatures, quality) {
  ...
  if (quality === 'color-groups') {
    add(
      'color-grouping',
      stats.mergedShapes,
      'Color Groups sengaja menggabungkan shape dengan paint identik menjadi satu layer warna.'
    );
    add(
      'color-group-z-order',
      stats.zOrderBarriers,
      'Warna yang muncul berselang-seling di z-order asli harus dikelompokkan; cek overlap antar warna.'
    );
  }
  return {
...
export function convertSvgToAlightXml(svgInput, rawOptions = {}) {
  ...
  const preserveFidelity = ['lossless', 'patch-clean', 'color-groups'].includes(options.quality);
  const preserveHierarchy = preserveFidelity && options.quality !== 'color-groups' && options.groupingMode === 'nested';
  ...
  if (options.quality === 'lossless' || options.quality === 'color-groups') {
    ...
    if (options.quality === 'color-groups') {
      const grouped = groupLosslessShapesByColor(losslessShapes);
      const colorLayers = grouped.groups;
      stats.groupedSourceShapes = losslessShapes.length;
      stats.colorGroups = colorLayers.length;
      stats.mergedShapes = Math.max(0, losslessShapes.length - colorLayers.length);
      stats.zOrderBarriers = grouped.zOrderBarriers;
      stats.outputShapes = colorLayers.length;
      stats.missingShapes = Math.max(0, stats.drawableElements - losslessShapes.length);

      if (stats.mergedShapes > 0) {
        warnings.add(`Color Groups menggabungkan ${losslessShapes.length} shape sumber menjadi ${colorLayers.length} layer berdasarkan paint identik.`);
      }
      if (stats.zOrderBarriers > 0) {
        warnings.add(`${stats.zOrderBarriers} pola warna muncul berselang-seling pada z-order asli. Grouping warna tetap dilakukan; periksa area overlap antar warna.`);
      }
      if (stats.missingShapes > 0) {
        warnings.add(`${stats.missingShapes} drawable SVG gagal masuk ke pipeline sebelum grouping warna.`);
      }
      ...
      const scene = [
        ...
        `<!-- Color Groups: Maximum Fidelity geometry parsing, then exact-paint layer grouping. -->`,
      ...
      return {
        ...
        colorGrouping: {
          engine: 'color-groups',
          sourceShapes: losslessShapes.length,
          colorLayers: colorLayers.length,
          mergedShapes: stats.mergedShapes,
          zOrderBarriers: stats.zOrderBarriers
        },
        validation: {
          enabled: false,
          sourceDrawable: stats.drawableElements,
          emittedShapes: losslessShapes.length,
        ...
        profile: {
          ...
          engine: 'color-groups',
          groupedByColor: true,
          preservesSourceOrder: stats.zOrderBarriers === 0,
