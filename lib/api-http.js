...
async function parseAndConvert(request, mode, authResult = null) {
  ...
  if (request.method === 'GET') {
    ...
    const options = {};
    const engine = url.searchParams.get('engine') || url.searchParams.get('quality');
    if (engine) options.quality = engine;

    for (const key of ['title', 'patchAreaPercent', 'protectThinPercent', 'duration', 'fps']) {
      const value = url.searchParams.get(key);
      if (value != null && value !== '') options[key] = value;
    }

    body = {
      svg: url.searchParams.get('svg'),
      options
    };
  } else {
    try {
...
export function createConvertHandler({ mode }) {
  ...
  return async function handleRequest(request) {
    try {
      ...
      const methodAllowed = mode === 'external'
      ...
      if (!methodAllowed) {
        return jsonResponse(request, {
          ok: false,
          error: mode === 'external' ? 'Gunakan GET atau POST.' : 'Gunakan POST.',
